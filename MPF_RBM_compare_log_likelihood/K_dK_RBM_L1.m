...
function [K, dK] = K_dK_RBM_L1( theta, Xin, lambda )
...
%    dK = dK + p1 * p2 / 2;
    ...
    % not sure which way the signs need to go here/
    K  = K  / nbatch + lambda*sum(sum(abs(penalty(:))));
    %dK = dK / nbatch + lambda*(penalty~=0).*penalty./abs(penalty);
    dK = dK / nbatch + lambda*sign(penalty);
