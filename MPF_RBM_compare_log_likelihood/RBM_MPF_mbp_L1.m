...
%
% this only differs from RBM_MPF.m in that it requires we enter in our
% previous W so that we can continue learning or not.
%
...
function [model Winit] = RBM_MPF_mbp_L1(d_vis, d_hid, Winit, X, lambda)
...
%for i = 1:n_learns
    learn_t = tic();
    Wmpfn = minFunc( @K_dK_RBM_L1, Wmpf(:), minf_options, X, lambda );
    Wmpf(:) = Wmpfn(:);
    %W_mpf{i} = Wmpf(:);
    learn_t = toc(learn_t)
